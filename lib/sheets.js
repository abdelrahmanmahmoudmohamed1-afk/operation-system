...
const SOURCES = {
  inventory: [
    { project:'Layana', id:process.env.GOOGLE_LAYANA_SPREADSHEET_ID || '1zi-I3q1wwVjLlYA1gG6Dz_O9FREZJTLSy_1VDeX07N0', sheet:'Layana Inventory Management', headerRow:2 },
    { project:'Mersea', id:process.env.GOOGLE_MERSEA_SPREADSHEET_ID || '13JaviUMCTKD433atOUd6-pqtHSdA6-6eAcMRw-2t-dg', sheet:'Mersea Inventory Management', headerRow:2 }
  ],
  clients: [
    { project:'Layana', id:process.env.GOOGLE_LAYANA_SPREADSHEET_ID || '1zi-I3q1wwVjLlYA1gG6Dz_O9FREZJTLSy_1VDeX07N0', sheet:'Layana Transaction', headerRow:2 },
    { project:'Mersea', id:process.env.GOOGLE_MERSEA_SPREADSHEET_ID || '13JaviUMCTKD433atOUd6-pqtHSdA6-6eAcMRw-2t-dg', sheet:'Mersea Transaction', headerRow:2 }
  ],
  eoi: [{ project:'Mersea', id:process.env.GOOGLE_EOI_SPREADSHEET_ID || process.env.GOOGLE_EOI_SHEETS || process.env.GOOGLE_EOI_SHEET || '1k0N-63K7tpinWHmODki7eLKXnH8k9wWetBUgKPgLOwg', sheet:process.env.GOOGLE_EOI_TAB_NAME || 'EOIS MERSEA', sheetAliases:['EOIS MERSEA','EOIS MERSEA Form','EOI Data',"EOI's",'EOI'], headerRow:1 }],
  orientation: [{ project:'', id:process.env.GOOGLE_ORIENTATION_SPREADSHEET_ID || process.env.GOOGLE_ORIENTATION_SHEET || '1uPPGfoYGSrGbWCCFGNuvd5lEwuo0aHEAYBXRZUMr_1g', sheet:process.env.GOOGLE_ORIENTATION_TAB_NAME || 'Orientation Sheet', sheetAliases:['Orientation Sheet','Orientation','Data','Sheet1'], headerRow:1 }],
  leads: [{ project:'', id:process.env.GOOGLE_LEADS_SPREADSHEET_ID || process.env.GOOGLE_LEADS_SHEET || '11MRmHkPZLMITmMU6vYuFk5u9mdo3qBtvN_eqHp5Bzys', sheet:process.env.GOOGLE_LEADS_TAB_NAME || 'Feedback Leads', headerRow:1 }]
};
...
function validateHeaders(kind,src,headers){
  const rules={
    ...
    orientation:[['Sales Agent','Sales Name','Sales'],['Date','Date and Time','Orientation Date']]
  };
...
async function readSource(src,kind){
  src={...src,id:spreadsheetId(src.id)};
  ...
  const job=(async()=>{
    let titles=[];
    try{
      const meta=await client().spreadsheets.get({spreadsheetId:src.id,fields:'sheets.properties.title'},{timeout:10000});
      titles=(meta.data.sheets||[]).map(x=>x.properties?.title).filter(Boolean);
    }catch(e){ throw new Error(`Spreadsheet ${src.id} could not be opened: ${e.message}`); }
    const preferred=[src.sheet,...(src.sheetAliases||[])].filter(Boolean);
    const candidates=[...new Set([...preferred,...titles])].filter(t=>titles.includes(t));
    if(!candidates.length) throw new Error(`Spreadsheet opened, but no readable tabs were found. Available tabs: ${titles.join(', ')}`);
    let lastError=null;
    for(const title of candidates){
      try{
        const res=await client().spreadsheets.values.get({spreadsheetId:src.id,range:a1Name(title),majorDimension:'ROWS',valueRenderOption:'FORMATTED_VALUE'},{timeout:10000});
        const values=res.data.values||[]; const hi=Math.max(0,(src.headerRow||1)-1); const headers=(values[hi]||[]).map(x=>String(x||'').trim());
        if(!headers.some(Boolean))throw new Error(`${title}: header row ${src.headerRow||1} is empty.`);
        validateHeaders(kind,{...src,sheet:title},headers);
        const rows=values.slice(hi+1).map((row,offset)=>({row,offset})).filter(({row})=>row.some(v=>String(v??'').trim()!=='' )).map(({row,offset})=>{const o={};headers.forEach((h,j)=>{if(h)o[h]=row[j]??'';});o.__rowNumber=hi+offset+2;return o;});
        sourceCache.set(cacheKey,{at:Date.now(),rows});
        return rows;
      }catch(e){ lastError=e; }
    }
    throw new Error(`No valid ${kind} tab found in spreadsheet. Tried: ${candidates.join(', ')}. Last error: ${lastError?.message||'unknown'}`);
...
export async function sheetLeads(){const all=await readMany('leads');if(all===null)return null;return all.map(({r},i)=>({id:`sheet-lead-${i}`,row_number:Number(r.__rowNumber||i+2),lead_key:String(pick(r,['S','Lead ID','Lead Id','ID'])),client_name:String(pick(r,['Client Name','Full Name','Name'])),phone:String(pick(r,['Client Phone Number','Client Phone','Phone/Mobile 1','Mobile','Phone'])),project:String(pick(r,['Project','Project Name','Project Name 1'])),sales_name:String(pick(r,['Sales Name','Assigned To','Sales'])),status:String(pick(r,['Lead Status','Status'],'Not Contacted')),stage:String(pick(r,['Lead Stage','Stage'],'New')),source:String(pick(r,['Lead Source','Lead Source Information','Source','Campaign Name'])),last_comment:String(pick(r,['Last Comment','Comment','Comments'])),assigned_at:pick(r,['Last Assigned Date','Assigned Date','Create Time']),status_changed_at:pick(r,['Last Comment Date','Status Changed At']),raw_data:r,updated_at:new Date().toISOString()}));}
export function sheetsEnabled(){return enabled();}

export async function sheetOrientation(){const all=await readMany('orientation');if(all===null)return null;return all.map(({r},i)=>({id:`sheet-orientation-${i}`,date:pick(r,['Date','Date and Time','Orientation Date']),sales:String(pick(r,['Sales Agent','Sales Name','Sales'])),manager:String(pick(r,['Sales Manager','Manager'])),director:String(pick(r,['Sales Director','Director','HOS'])),company:String(pick(r,['Broker Company','Broker Company Name','Company','Company Name'])),activity:String(pick(r,['Activity Type','Activity','Orientation'])),feedback:String(pick(r,['Feedback','Status','Result'])),raw_data:r}));}
