...
function a1Name(name){return `'${String(name).replace(/'/g,"''")}'!A:ZZ`;}
async function readSource(src){
  const res=await client().spreadsheets.values.get({spreadsheetId:src.id,range:a1Name(src.sheet),majorDimension:'ROWS',valueRenderOption:'FORMATTED_VALUE'});
  const values=res.data.values||[]; const hi=Math.max(0,(src.headerRow||1)-1); const headers=(values[hi]||[]).map(x=>String(x||'').trim());
  return values.slice(hi+1).filter(row=>row.some(v=>String(v??'').trim()!=='' )).map((row,i)=>{const o={};headers.forEach((h,j)=>{if(h)o[h]=row[j]??'';});o.__rowNumber=hi+i+2;return o;});
}
async function readMany(kind){if(!enabled())return null;const groups=await Promise.all(SOURCES[kind].map(async src=>({src,rows:await readSource(src)})));return groups.flatMap(({src,rows})=>rows.map(r=>({src,r})));}
