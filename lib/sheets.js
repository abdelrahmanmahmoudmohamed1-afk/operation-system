...
async function readSource(src,kind){
  ...
  const job=(async()=>{
    const res=await client().spreadsheets.values.get({spreadsheetId:src.id,range:a1Name(src.sheet),majorDimension:'ROWS',valueRenderOption:'FORMATTED_VALUE'},{timeout:15000});
