...
function withTimeout(promise, ms, label = 'Google Sheets read') {
  let timer;
  return Promise.race([
    promise,
...
async function readSource(src,kind){
  ...
  const job=withTimeout((async()=>{
    ...
    return rows;
  })(),12000,`Google Sheets "${src.sheet}"`).finally(()=>pendingReads.delete(cacheKey));
  pendingReads.set(cacheKey,job);
  return job;
}
export async function sheetsDiagnostics(){
  if(!enabled())return {configured:false,sources:[]};
  const all=Object.entries(SOURCES).flatMap(([kind,list])=>list.map(src=>({kind,src})));
  const sources=await Promise.all(all.map(async({kind,src})=>{
    try{
      const res=await withTimeout(client().spreadsheets.values.get({spreadsheetId:src.id,range:a1Name(src.sheet),majorDimension:'ROWS',valueRenderOption:'FORMATTED_VALUE'},{timeout:10000}),12000,`Google Sheets "${src.sheet}"`);
      const values=res.data.values||[];
      const hi=Math.max(0,(src.headerRow||1)-1);
      const headers=(values[hi]||[]).map(x=>String(x||'').trim());
      const dataRowsRaw=values.slice(hi+1);
      const dataRowsNonEmpty=dataRowsRaw.filter(row=>row.some(v=>String(v??'').trim()!==''));
      let headerError=null;
      try{ if(!headers.some(Boolean)) throw new Error('header row is empty'); validateHeaders(kind,src,headers); }catch(e){ headerError=e.message; }
      return {kind,project:src.project,sheet:src.sheet,spreadsheetId:src.id,headerRowConfigured:src.headerRow||1,totalValueRowsFetched:values.length,headersFound:headers,headerError,dataRowsAfterHeaderSkip:dataRowsRaw.length,dataRowsNonEmpty:dataRowsNonEmpty.length};
    }catch(e){
      return {kind,project:src.project,sheet:src.sheet,spreadsheetId:src.id,error:e.message};
    }
  }));
