...
function getSystemInfo_() {
  ...
  return {
    ...
    version: 'Enterprise 5.9 AI Agent',
    apiVersion: '2026.08.09.2',
    backendBuild: '5.9.0',
    projects: (PROJECT_SOURCES || []).map(function(x){ return x.key; }),
    features: ['unified-projects','users','audit','leads','contract-pdf','achievement','housing-eoi','architectural-drawings','document-coverage','ai-agent-v2','ai-tool-calling','ai-document-retrieval','ai-navigation','api-manifest','stable-inventory-path'],
    actions: actions
  };
}
