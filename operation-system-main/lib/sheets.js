...
const SOURCES = {
  ...
  eoi: [{ project:'Mersea', id:process.env.GOOGLE_EOI_SPREADSHEET_ID || process.env.GOOGLE_EOI_SHEETS || process.env.GOOGLE_EOI_SHEET || '1k0N-63K7tpinWHmODki7eLKXnH8k9wWetBUgKPgLOwg', sheet:process.env.GOOGLE_EOI_TAB_NAME || 'EOIS MERSEA', sheetAliases:['EOIS MERSEA','EOI Data',"EOI's",'EOI'], headerRow:1 }],
  ...
  leads: [{ project:'', id:process.env.GOOGLE_LEADS_SPREADSHEET_ID || process.env.GOOGLE_LEADS_SHEET || '19KuqLx1AXw2cg2qTzFJvfwR0_ljDsUUh', sheet:process.env.GOOGLE_LEADS_TAB_NAME || 'Feedback Leads', headerRow:1 }]
};
...
export async function sheetOrientation(){const all=await readMany('orientation');if(all===null)return null;return all.map(({r},i)=>({id:`sheet-orientation-${i}`,date:pick(r,['Date','Date and Time','Orientation Date']),sales:String(pick(r,['Sales Agent','Sales Name','Sales'])),manager:String(pick(r,['Sales Manager','Manager'])),director:String(pick(r,['Sales Director','Director','HOS'])),company:String(pick(r,['Broker Company','Company','Company Name'])),activity:String(pick(r,['Activity Type','Activity','Orientation'])),feedback:String(pick(r,['Feedback','Status','Result'])),raw_data:r}));}
