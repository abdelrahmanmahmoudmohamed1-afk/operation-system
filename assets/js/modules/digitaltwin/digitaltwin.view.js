...
export function renderStage(rows){
  ...
  return `<div class="twin-world">${Object.values(groups).map(g=>{
    ...
    return `<article class="twin-building-card" data-building="${escapeHtml(g.building)}">
      <header><div><span>${escapeHtml(projectLabel(g.project))}</span><h3>${escapeHtml(g.building)}</h3></div><strong>${occupancy}%</strong></header>
...
export function renderInspector(unit, allRows, planState=null){
  ...
  return `<div class="twin-detail-head"><div><span class="eyebrow">Unit intelligence</span><h2>${escapeHtml(unit.unitCode||'Unit')}</h2><p>${escapeHtml(projectLabel(unit.project||''))} · ${escapeHtml(unit.building||'')} · Floor ${escapeHtml(unit.floor||'—')}</p></div><span class="badge badge-${slug(unit.status)}">${statusLabel(unit.status)}</span></div>
