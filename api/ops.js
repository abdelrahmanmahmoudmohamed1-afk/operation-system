...
import { sheetInventory, sheetClients, sheetEOI, sheetLeads, sheetOrientation, sheetsEnabled } from '../lib/sheets.js';
...
const ACTION_PERMISSION_ROUTES={
  getSystemInfo:['settings'],runDiagnostics:['settings'],recordUserActivity:['settings'],
  getSales:['crm','salesoperations','reports'],getCompanies:['crm','salesoperations','reports'],getManagerDirector:['crm','salesoperations'],
  operationAiChat:['commandcenter','overview','dashboard','crm','inventory','digitaltwin','reports','eoi','leads','contracts','documents','payment','users','settings','analytics','tasks','quality','achievement','chat'],
  ...
  getChatBootstrap:['chat'],getChatMessages:['chat'],prepareChatAttachmentUpload:['chat'],sendChatMessage:['chat'],sendChatAnnouncement:['chat'],createChatConversation:['chat'],markChatRead:['chat'],
...
function canProfileAccess(profile,route){if(!profile) return false;const role=String(profile.role||'user').toLowerCase();if(role==='admin')return true;const p=Array.isArray(profile.permissions)?profile.permissions:DEFAULT_USER_PERMISSIONS;return p.includes(route);}
function requireActionPermission(user,action){const routes=ACTION_PERMISSION_ROUTES[action];if(!routes||!routes.length){const e=new Error('PERMISSION_DENIED');e.status=403;e.code='MODULE_PERMISSION_REQUIRED';throw e;}const ok=routes.some(r=>canProfileAccess(user?.profile,r));if(!ok){const e=new Error('PERMISSION_DENIED');e.status=403;e.code='MODULE_PERMISSION_REQUIRED';throw e;}}
...
async function diagnostics(user=null){
  ...
  if(sheetsEnabled()){const [u,c,e,l]=await Promise.all([safeSheet('inventory'),safeSheet('clients'),safeSheet('eoi'),safeSheet('leads')]);checks.googleSheets.inventoryRows=u?.length??null;checks.googleSheets.clientRows=c?.length??null;checks.googleSheets.eoiRows=e?.length??null;checks.googleSheets.leadsRows=l?.length??null;}
...
async function handle(action,payload){
  ...
  switch(action){
    ...
    case 'getClientFormBootstrap':{const[u,c,sp]=await Promise.all([rows('inventory_units',{}),rows('clients',{}),admin.from('sales_people').select('name').eq('role','sales').eq('is_active',true)]);const masterSales=sp.data||[];return{sales:(masterSales.length?masterSales.map(x=>x.name):uniq(c.map(x=>x.sales_name))).filter(Boolean).sort(),lists:{projects:uniq(u.map(x=>x.project)).sort(),statuses:uniq(c.map(x=>x.status)).filter(Boolean).sort(),nationalities:uniq(c.map(x=>rawPick(x,['Client Nationality','Nationality']))).filter(Boolean).sort(),sourceOptions:['Personal','Direct','Non-Direct','Broker','Referral','Freelance','Digital','Call Center','Visit','Visit (Site)'],paymentMethods:['Cash','Cheque','Transfer']}};}
    case 'getSales':{const {data}=await admin.from('sales_people').select('name').eq('role','sales').eq('is_active',true).order('name');if(data?.length)return data.map(x=>x.name);return uniq((await rows('clients',{})).map(x=>x.sales_name)).filter(Boolean).sort();}
    case 'getCompanies':{const c=await rows('clients',{});return uniq(c.map(x=>rawPick(x,['Broker Company','Company'])).filter(Boolean)).sort();}
    case 'getManagerDirector':{const salesName=String(payload.salesName||payload.data?.salesName||'').trim(),sales=norm(salesName);const {data:p}=await admin.from('sales_people').select('manager_name,director_name').ilike('name',salesName).maybeSingle();if(p)return{manager:p.manager_name||'',director:p.director_name||''};const c=await rows('clients',{});const match=c.find(x=>norm(x.sales_name)===sales);return{manager:match?String(rawPick(match,['Sales Manager','Manager'])||''):'',director:match?String(rawPick(match,['Sales Director','Director','HOS'])||''):''};}
    ...
    case 'getSalesOrganization':{
      const [peopleR,targetsR,clients,leads]=await Promise.all([admin.from('sales_people').select('*').order('name'),admin.from('sales_targets').select('*'),rows('clients',{}),rows('leads',{})]);
      if(peopleR.error)throw peopleR.error;if(targetsR.error)throw targetsR.error;
      ...
      const targetMap=new Map((targetsR.data||[]).filter(t=>new Date(t.period_start)<=now&&new Date(t.period_end)>=now).map(t=>[t.sales_person_id,n(t.target_value)]));
      const perf=new Map();for(const c of clients){if(!['reserved','contracted','sold'].includes(norm(c.status)))continue;const d=parseDateValue(achievementDate(c));if(!d||d<monthStart||d>monthEnd)continue;const k=norm(c.sales_name),x=perf.get(k)||{value:0,units:0};x.value+=n(c.value);x.units++;perf.set(k,x);}
      const leadMap=new Map();for(const l of leads){const k=norm(l.sales_name),x=leadMap.get(k)||0;leadMap.set(k,x+1);}
      const people=(peopleR.data||[]).map(p=>{const a=perf.get(norm(p.name))||{value:0,units:0},target=targetMap.get(p.id)||0;return{id:p.id,name:p.name,role:p.role,managerName:p.manager_name,directorName:p.director_name,joinDate:p.join_date,leaveDate:p.leave_date,isActive:p.is_active,target,achievement:a.value,achievementUnits:a.units,achievementPct:target?a.value/target*100:0,leads:leadMap.get(norm(p.name))||0};});
      return{people,summary:{activeSales:people.filter(x=>x.role==='sales'&&x.isActive).length,target:people.reduce((a,x)=>a+x.target,0),achievement:people.reduce((a,x)=>a+x.achievement,0),leads:leads.length}};
    }
    case 'saveSalesPerson':{
      const d=payload.data||{};if(!String(d.name||'').trim())throw new Error('Sales person name is required.');
      const rec={name:String(d.name).trim(),role:['sales','manager','director'].includes(d.role)?d.role:'sales',manager_name:d.managerName||'',director_name:d.directorName||'',join_date:d.joinDate||null,leave_date:d.leaveDate||null,is_active:d.isActive!==false,updated_at:new Date().toISOString()};
      const {data:person,error}=await admin.from('sales_people').upsert(rec,{onConflict:'name'}).select('*').single();if(error)throw error;
      if(n(d.target)>0){const now=new Date(),start=`${now.getFullYear()}-${String(now.getMonth()+1).padStart(2,'0')}-01`,end=new Date(now.getFullYear(),now.getMonth()+1,0).toISOString().slice(0,10);const {error:te}=await admin.from('sales_targets').upsert({sales_person_id:person.id,period_type:'monthly',period_start:start,period_end:end,target_value:n(d.target),updated_at:new Date().toISOString()},{onConflict:'sales_person_id,period_type,period_start'});if(te)throw te;}
      await audit(user,'save_sales_person','salesoperations',{name:person.name,role:person.role});return person;
    }
    case 'saveSalesTarget':{const d=payload.data||{};const {data,error}=await admin.from('sales_targets').upsert({sales_person_id:d.salesPersonId,period_type:d.periodType||'monthly',period_start:d.periodStart,period_end:d.periodEnd,target_value:n(d.targetValue),updated_at:new Date().toISOString()},{onConflict:'sales_person_id,period_type,period_start'}).select('*').single();if(error)throw error;return data;}
    case 'getOrientationData':{
    ...
    case 'getLeadsData':{const all=await rows('leads',filters);const now=Date.now();all.forEach(x=>{const raw=x.raw_data||{};x.assigned_at=x.assigned_at||raw['Assigned Date']||raw['Lead Date']||raw['Date']||x.updated_at;x.status_changed_at=x.status_changed_at||raw['Status Changed At']||x.assigned_at;x.leadAgeDays=Math.max(0,Math.floor((now-(parseDateValue(x.assigned_at)?.getTime()||now))/86400000));x.statusAgeDays=Math.max(0,Math.floor((now-(parseDateValue(x.status_changed_at)?.getTime()||now))/86400000));});let r=all;const st=String(filters.status||'ALL');if(st&&st!=='ALL')r=r.filter(x=>norm(x.status)===norm(st));const search=norm(filters.search||'');if(search)r=r.filter(x=>norm(`${x.client_name} ${x.phone} ${x.project} ${x.sales_name} ${x.status} ${x.stage} ${x.source} ${x.last_comment} ${JSON.stringify(x.raw_data||{})}`).includes(search));return{rows:r.map(x=>({...x.raw_data,id:x.id,rowNumber:x.row_number,clientName:x.client_name,phone:x.phone,project:x.project,salesName:x.sales_name,status:x.status,stage:x.stage,source:x.source,lastComment:x.last_comment,assignedAt:x.assigned_at,leadAgeDays:x.leadAgeDays,statusAgeDays:x.statusAgeDays})),statuses:uniq(all.map(x=>x.status)).filter(Boolean).sort(),meta:{total:r.length,all:all.length}};}
    ...
    case 'importLeads':{const incoming=payload.data?.rows||[];const mapped=incoming.map((d,i)=>({row_number:Number(d.rowNumber||d['Row Number']||Date.now()+i),lead_key:String(d.leadId||d['Lead ID']||d.phone||d.Mobile||d.Phone||`${Date.now()}-${i}`).replace(/\s/g,''),client_name:d.clientName||d['Client Name']||d.Name||'',phone:String(d.phone||d.Mobile||d.Phone||''),project:d.project||d.Project||'',sales_name:d.salesName||d['Sales Name']||d['Assigned To']||'',status:d.status||d['Lead Status']||d.Status||'Not Contacted',stage:d.stage||d.Stage||'New',source:d.source||d.Source||'',last_comment:d.lastComment||d['Last Comment']||'',assigned_at:d.assignedAt||d['Assigned Date']||d.Date||new Date().toISOString(),status_changed_at:d.statusChangedAt||d['Status Changed At']||d['Assigned Date']||d.Date||new Date().toISOString(),last_activity_at:new Date().toISOString(),raw_data:d,updated_at:new Date().toISOString()}));if(mapped.length){const{error}=await admin.from('leads').upsert(mapped,{onConflict:'row_number'});if(error)throw error;}await audit(user,'import_leads','leads',{count:mapped.length});return{imported:mapped.length,updated:0,skipped:0};}
    case 'getChatBootstrap':{
      const {data:profiles,error:pe}=await admin.from('profiles').select('id,username,full_name,role,is_active').eq('is_active',true).order('full_name');if(pe)throw pe;
      const {data:members,error:me}=await admin.from('chat_members').select('conversation_id,last_read_at').eq('user_id',user.id);if(me)throw me;
      const ids=(members||[]).map(x=>x.conversation_id);if(!ids.length)return{me:{id:user.id,email:user.email,isAdmin:String(user.profile?.role||'').toLowerCase()==='admin'},users:(profiles||[]).filter(x=>x.id!==user.id).map(x=>({...x,name:x.full_name||x.username})),conversations:[]};
      const {data:convs,error:ce}=await admin.from('chat_conversations').select('*').in('id',ids).order('updated_at',{ascending:false});if(ce)throw ce;
      const {data:allMembers}=await admin.from('chat_members').select('conversation_id,user_id').in('conversation_id',ids);
      const {data:msgs}=await admin.from('chat_messages').select('id,conversation_id,sender_id,body,created_at,message_type,mention_user_ids').in('conversation_id',ids).order('created_at',{ascending:false}).limit(1500);
      const pm=new Map((profiles||[]).map(x=>[x.id,x])),own=new Map((members||[]).map(x=>[x.conversation_id,x]));
      const out=(convs||[]).map(c=>{const cm=(allMembers||[]).filter(x=>x.conversation_id===c.id);const others=cm.filter(x=>x.user_id!==user.id).map(x=>pm.get(x.user_id)).filter(Boolean);const name=c.is_group?(c.name||'Group'):((others[0]?.full_name||others[0]?.username)||'Direct chat');const list=(msgs||[]).filter(x=>x.conversation_id===c.id);const last=list[0],read=own.get(c.id)?.last_read_at;const fresh=list.filter(x=>x.sender_id!==user.id&&(!read||new Date(x.created_at)>new Date(read)));const unread=fresh.length;const mentionUnread=fresh.filter(x=>Array.isArray(x.mention_user_ids)&&x.mention_user_ids.includes(user.id)).length;const announcementUnread=fresh.filter(x=>x.message_type==='announcement').length;return{id:c.id,name,isGroup:c.is_group,isAnnouncementChannel:Boolean(c.is_announcement_channel),lastMessage:last?.message_type==='announcement'?`📢 ${last?.body||'Announcement'}`:(last?.body||''),lastAt:last?.created_at||c.updated_at,unread,mentionUnread,announcementUnread};}).sort((a,b)=>new Date(b.lastAt)-new Date(a.lastAt));
      return{me:{id:user.id,email:user.email,isAdmin:String(user.profile?.role||'').toLowerCase()==='admin'},users:(profiles||[]).filter(x=>x.id!==user.id).map(x=>({...x,name:x.full_name||x.username})),conversations:out};
    }
    case 'getChatMessages':{
      const id=String(payload.data?.conversationId||'');const {data:member}=await admin.from('chat_members').select('conversation_id').eq('conversation_id',id).eq('user_id',user.id).maybeSingle();if(!member)throw Object.assign(new Error('You are not a member of this chat.'),{status:403});
      const [{data:c},{data:ms,error},{data:members},{data:profiles}]=await Promise.all([admin.from('chat_conversations').select('*').eq('id',id).single(),admin.from('chat_messages').select('*').eq('conversation_id',id).order('created_at',{ascending:true}).limit(1000),admin.from('chat_members').select('user_id,last_read_at').eq('conversation_id',id),admin.from('profiles').select('id,username,full_name,role')]);if(error)throw error;
      const pm=new Map((profiles||[]).map(x=>[x.id,x])),others=(members||[]).filter(x=>x.user_id!==user.id),messageIds=(ms||[]).map(x=>x.id);let atts=[];if(messageIds.length){const {data:a,error:ae}=await admin.from('chat_attachments').select('*').in('message_id',messageIds).order('created_at');if(ae)throw ae;atts=a||[];}
      const signed=[];for(const a of atts){const {data}=await admin.storage.from(a.bucket||'chat-attachments').createSignedUrl(a.storage_path,3600);signed.push({...a,url:data?.signedUrl||''});}
      const attMap=new Map();signed.forEach(a=>{if(!attMap.has(a.message_id))attMap.set(a.message_id,[]);attMap.get(a.message_id).push({id:a.id,fileName:a.file_name,mimeType:a.mime_type,fileSize:a.file_size,url:a.url});});
      const msgMap=new Map((ms||[]).map(x=>[String(x.id),x]));const name=c?.is_group?(c.name||'Group'):((pm.get(others[0]?.user_id)?.full_name||pm.get(others[0]?.user_id)?.username)||'Direct chat');
      return{name,isGroup:Boolean(c?.is_group),isAnnouncementChannel:Boolean(c?.is_announcement_channel),members:(members||[]).map(m=>{const p=pm.get(m.user_id)||{};return{id:m.user_id,name:p.full_name||p.username||'User',username:p.username||'',role:p.role||'user'};}),messages:(ms||[]).map(x=>{const sp=pm.get(x.sender_id)||{},seenMembers=others.filter(m=>m.last_read_at&&new Date(m.last_read_at)>=new Date(x.created_at)),seen=x.sender_id===user.id&&others.length>0&&seenMembers.length===others.length;const replyRaw=x.reply_to_id?msgMap.get(String(x.reply_to_id)):null,replyProfile=replyRaw?pm.get(replyRaw.sender_id)||{}:{};return{id:x.id,senderId:x.sender_id,senderName:sp.full_name||sp.username||'User',senderUsername:sp.username||'',text:x.body||'',createdAt:x.created_at,seen,seenCount:seenMembers.length,recipientCount:others.length,messageType:x.message_type||'text',mentions:Array.isArray(x.mention_user_ids)?x.mention_user_ids:[],attachments:attMap.get(x.id)||[],reply:replyRaw?{id:replyRaw.id,senderName:replyProfile.full_name||replyProfile.username||'User',text:replyRaw.body||'',messageType:replyRaw.message_type||'text'}:null};})};
    }
    case 'prepareChatAttachmentUpload':{
      const d=payload.data||{},id=String(d.conversationId||''),fileName=String(d.fileName||'attachment').replace(/[\\/\x00-\x1f]/g,'_').slice(0,180),mimeType=String(d.mimeType||'application/octet-stream').toLowerCase(),fileSize=Number(d.fileSize||0);const {data:member}=await admin.from('chat_members').select('conversation_id').eq('conversation_id',id).eq('user_id',user.id).maybeSingle();if(!member)throw Object.assign(new Error('You are not a member of this chat.'),{status:403});
      const allowed=['application/pdf','image/jpeg','image/png','image/webp','text/plain','text/csv','application/vnd.openxmlformats-officedocument.spreadsheetml.sheet','application/vnd.ms-excel','application/vnd.openxmlformats-officedocument.wordprocessingml.document'];if(!allowed.includes(mimeType))throw new Error('Unsupported attachment type. Use PDF, image, Excel, Word, TXT or CSV.');if(!fileSize||fileSize>15*1024*1024)throw new Error('Attachment must be 15 MB or smaller.');const ext=(fileName.split('.').pop()||'bin').replace(/[^a-z0-9]/gi,'').slice(0,10)||'bin',path=`chat/${id}/${user.id}/${Date.now()}-${Math.random().toString(36).slice(2,10)}.${ext}`;const {data,error}=await admin.storage.from('chat-attachments').createSignedUploadUrl(path);if(error)throw error;return{bucket:'chat-attachments',storagePath:path,signedUrl:data?.signedUrl||'',token:data?.token||'',fileName,mimeType,fileSize};
    }
    case 'sendChatMessage':{
      const d=payload.data||{},id=String(d.conversationId||''),text=String(d.text||'').trim(),replyToId=d.replyToId?Number(d.replyToId):null,attachments=Array.isArray(d.attachments)?d.attachments.slice(0,5):[];if(!text&&!attachments.length)throw new Error('Message or attachment is required.');if(text.length>4000)throw new Error('Message is too long.');const {data:member}=await admin.from('chat_members').select('conversation_id').eq('conversation_id',id).eq('user_id',user.id).maybeSingle();if(!member)throw Object.assign(new Error('You are not a member of this chat.'),{status:403});const {data:conv}=await admin.from('chat_conversations').select('is_announcement_channel').eq('id',id).single();if(conv?.is_announcement_channel&&String(user.profile?.role||'').toLowerCase()!=='admin')throw Object.assign(new Error('Only Admins can post in the announcement channel.'),{status:403});
      if(replyToId){const {data:reply}=await admin.from('chat_messages').select('id').eq('id',replyToId).eq('conversation_id',id).maybeSingle();if(!reply)throw new Error('Reply target is not available in this conversation.');}
      const {data:memberRows}=await admin.from('chat_members').select('user_id').eq('conversation_id',id);const memberIds=new Set((memberRows||[]).map(x=>x.user_id));const {data:profiles}=await admin.from('profiles').select('id,username').in('id',[...memberIds]);const usernameMap=new Map((profiles||[]).map(p=>[String(p.username||'').toLowerCase(),p.id]));const mentionIds=[];for(const m of text.matchAll(/@([A-Za-z0-9._-]{2,50})/g)){const mid=usernameMap.get(String(m[1]).toLowerCase());if(mid&&mid!==user.id&&!mentionIds.includes(mid))mentionIds.push(mid);}
      const {data:msg,error}=await admin.from('chat_messages').insert({conversation_id:id,sender_id:user.id,body:text,message_type:'text',reply_to_id:replyToId,mention_user_ids:mentionIds}).select('*').single();if(error)throw error;
      if(attachments.length){const rows=[];for(const a of attachments){const path=String(a.storagePath||'');if(!path.startsWith(`chat/${id}/${user.id}/`))throw new Error('Invalid attachment path.');rows.push({message_id:msg.id,bucket:'chat-attachments',storage_path:path,file_name:String(a.fileName||'attachment').slice(0,180),mime_type:String(a.mimeType||'application/octet-stream').slice(0,120),file_size:Number(a.fileSize||0),uploaded_by:user.id});}const {error:ae}=await admin.from('chat_attachments').insert(rows);if(ae)throw ae;}
      await admin.from('chat_conversations').update({updated_at:new Date().toISOString()}).eq('id',id);return{...msg,mentionIds};
    }
    case 'sendChatAnnouncement':{
      const adminUser=await requireAdmin(payload.token),d=payload.data||{},text=String(d.text||'').trim();if(!text)throw new Error('Announcement text is required.');if(text.length>4000)throw new Error('Announcement is too long.');let conversationId=String(d.conversationId||'');
      if(d.scope==='all'||!conversationId){let {data:c}=await admin.from('chat_conversations').select('*').eq('is_announcement_channel',true).maybeSingle();if(!c){const r=await admin.from('chat_conversations').insert({name:'Admin Announcements',is_group:true,is_announcement_channel:true,created_by:adminUser.id}).select('*').single();if(r.error)throw r.error;c=r.data;}conversationId=c.id;const {data:active}=await admin.from('profiles').select('id').eq('is_active',true);if(active?.length){const {error:me}=await admin.from('chat_members').upsert(active.map(p=>({conversation_id:conversationId,user_id:p.id})),{onConflict:'conversation_id,user_id',ignoreDuplicates:true});if(me)throw me;}}
      const {data:member}=await admin.from('chat_members').select('conversation_id').eq('conversation_id',conversationId).eq('user_id',adminUser.id).maybeSingle();if(!member)throw Object.assign(new Error('Admin must be a member of the selected conversation.'),{status:403});const {data:msg,error}=await admin.from('chat_messages').insert({conversation_id:conversationId,sender_id:adminUser.id,body:text,message_type:'announcement',mention_user_ids:[]}).select('*').single();if(error)throw error;await admin.from('chat_conversations').update({updated_at:new Date().toISOString()}).eq('id',conversationId);await audit(adminUser,'chat_announcement','chat',{conversationId,scope:d.scope==='all'?'all':'conversation'});return{conversationId,messageId:msg.id};
    }
    case 'createChatConversation':{const d=payload.data||{},memberIds=[...new Set([user.id,...(d.members||[]).map(String)])];if(memberIds.length<2)throw new Error('Select at least one user.');const isGroup=Boolean(d.isGroup||memberIds.length>2),name=String(d.name||'').trim();if(!isGroup&&memberIds.length===2){const {data:mine}=await admin.from('chat_members').select('conversation_id').eq('user_id',user.id);for(const x of mine||[]){const {data:mm}=await admin.from('chat_members').select('user_id').eq('conversation_id',x.conversation_id);if((mm||[]).length===2&&memberIds.every(uid=>(mm||[]).some(z=>z.user_id===uid)))return{id:x.conversation_id};}}const {data:c,error}=await admin.from('chat_conversations').insert({name:isGroup?(name||'New Group'):null,is_group:isGroup,created_by:user.id}).select('*').single();if(error)throw error;const {error:ie}=await admin.from('chat_members').insert(memberIds.map(uid=>({conversation_id:c.id,user_id:uid,last_read_at:uid===user.id?new Date().toISOString():null})));if(ie)throw ie;return{id:c.id};}
    case 'markChatRead':{const id=String(payload.data?.conversationId||'');const {error}=await admin.from('chat_members').update({last_read_at:new Date().toISOString()}).eq('conversation_id',id).eq('user_id',user.id);if(error)throw error;return{success:true};}
    ...
    case 'createSystemUser':{
      ...
      const requestedRole=String(d.role||'user').toLowerCase();const role=['admin','operations','sales'].includes(requestedRole)?requestedRole:'user';
      const profile={id:created.user.id,email,username,full_name:d.name||d.fullName||username,role,is_active:d.active!==false,manager:d.manager||'',director:d.director||'',mobile:d.mobile||'',permissions:role==='admin'?null:cleanPermissions(d.permissions),assigned_sales_name:d.assignedSalesName||''};
      const{error:pErr}=await admin.from('profiles').insert(profile);if(pErr){try{await admin.auth.admin.deleteUser(created.user.id);}catch{}throw pErr;}
    ...
    case 'updateSystemUser':{
      ...
      const requestedRole=String(d.role||'user').toLowerCase();const role=['admin','operations','sales'].includes(requestedRole)?requestedRole:'user';const active=d.active!==false;
