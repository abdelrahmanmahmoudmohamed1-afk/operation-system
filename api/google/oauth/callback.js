...
async function gmailCallbackFetch(request){
  try{
    // Gmail OAuth state/tokens are stored through Supabase's HTTPS API.
    // Do not block the OAuth callback on the optional direct-Postgres schema check.
    const url=new URL(request.url);const code=url.searchParams.get('code');const state=url.searchParams.get('state');if(!code||!state)throw new Error('Missing OAuth code/state.');
...
export default { fetch: gmailCallbackFetch };
